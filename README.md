...
| [Jogo da Memória](Games/numerandus/jogo-da-memoria/) | — | 🚧 jogável, [pendências declaradas](Games/numerandus/jogo-da-memoria/README.md#pendências-conhecidas) |
...
| `node tools/pages-index.mjs` | Gera o `index.html` da raiz (a capa que lista os jogos) e o `__jogos.json` da raiz (mesma lista, estática, para o seletor do `ava-teste.html` funcionar sob o GitHub Pages). Rode depois de criar um jogo novo — a lista é medida em `Games/`, não escrita à mão |
...
| `node tools/captura-cena.mjs <jogo\|.html\|url> [opções]` | Screenshot de qualquer cena, renderizada de verdade em navegador headless — pra conferir contraste, layout ou um estado difícil de alcançar tocando (ex.: forçar um tabuleiro inteiro virado). Ver o cabeçalho do arquivo |
