...
>
> Slug: `{{SLUG}}` · Criado em: {{DATA}} · Motor: v{{VERSAO_MOTOR}}
...
- [ ] `node tools/verificar-independencia.mjs {{SLUG}}` **aprovado**
- [ ] `node tools/teste-entrega-avulsa.mjs {{SLUG}}` **aprovado** (simula a publicação)
...
- [ ] `node tools/build.mjs {{SLUG}}` rodado por último (motor atualizado na cópia)
...
- [ ] Pasta copiada para **fora** do projeto, servida de uma subpasta qualquer e testada
      (`node tools/teste-entrega-avulsa.mjs {{SLUG}}` faz isso automaticamente)
