...
1. o nome do arquivo é `amarelo.mp3`, e é o próprio `id` declarado em `config.cores.amarelo.som`;
2. a duração de 0,97 s é compatível com a palavra "amarelo" (quatro sílabas);
...
**Isto não é o arquivo de 2013.** O `amarelo.mp3` da aula original tem outro formato (bitrate
menor, ~8 KB) e outro hash. Este é maior e mais nítido — dá para supor que é uma regravação,
mas **é suposição**: ninguém confirmou ainda que a palavra dita é "amarelo" e não outra coisa.
