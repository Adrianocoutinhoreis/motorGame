...
1. o nome do arquivo é `azul.mp3`, e é o próprio `id` declarado em `config.cores.azul.som`;
2. a duração de 0,86 s é compatível com a palavra "azul" (duas sílabas);
...
**Isto não é o arquivo de 2013.** O `azul.mp3` da aula original tem outro formato (bitrate
menor, ~7 KB) e outro hash. Este é maior e mais nítido — dá para supor que é uma regravação,
mas **é suposição**: ninguém confirmou ainda que a palavra dita é "azul" e não outra coisa.
