...
1. o nome do arquivo é `laranja.mp3`, e é o próprio `id` declarado em `config.cores.laranja.som`;
2. a duração de 1,07 s — a maior das oito — é compatível com "laranja" (três sílabas, mais longa
   que "azul" ou "rosa");
...
**Isto não é o arquivo de 2013.** O `laranja.mp3` da aula original tem outro formato (bitrate
menor, ~7 KB) e outro hash. Este é maior e mais nítido — dá para supor que é uma regravação,
mas **é suposição**: ninguém confirmou ainda que a palavra dita é "laranja" e não outra coisa.
