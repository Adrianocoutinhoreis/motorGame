...
| | |
|---|---|
| Arquivo | `assets/audio/dez.mp3` |
| `id` no motor | `dez` |
| Tipo | Narração — nome de número |
| Formato | MP3 MPEG2 Layer III, 40 kbps, 22 050 Hz, mono |
| Duração | **0,47 s** (18 frames) |
| SHA-256 (16 primeiros) | `442c5222ca90dc45` |
...
1. o nome do arquivo é `dez.mp3`;
2. no código da aula original, os arquivos seis..dez existiam na pasta original mas o código antigo nunca os carregava;
3. duração de 0,47 s — compatível com uma única palavra curta;
...
Nível 2 ("Números 6 a 10"), 5º bloco da torre (símbolo **10**). Tocado no canal `speech` no instante em que o bloco encaixa.
