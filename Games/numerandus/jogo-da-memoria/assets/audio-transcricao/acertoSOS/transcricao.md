...
Jogo da Velha, no Jogo da Ordenação e no Encaixe Certo — reaproveitado aqui de
propósito: por não ter fala, o mesmo efeito serve a qualquer jogo da coleção.

## Onde é usado no jogo

Canal `music` (não `sfx`), via `config.audio.vitoria`, disparado pela `ResultScreen`
do motor ao entrar na tela de resultado — este jogo nunca tem derrota (jogo da
memória solo, sempre termina em vitória), então `audio.derrota` continua `null` e
`erroSOS` não é usado aqui.
