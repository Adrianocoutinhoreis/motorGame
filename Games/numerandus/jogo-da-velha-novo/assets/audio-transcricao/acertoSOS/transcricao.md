...
Formas, no Jogo dos Blocos, no Jogo das Cores e no Bingo da Adição e Subtração —
reaproveitado aqui de propósito, não copiado às cegas: por não ter fala, o mesmo
efeito serve a qualquer jogo da coleção.

## Onde é usado no jogo

Canal `sfx`, via `config.audio.vitoria`. Diferente do Bingo, aqui não existe
comemoração própria na cena de jogo — quem dispara o som é a `ResultScreen` do
motor, uma única vez, ao entrar na tela de resultado por vitória. Sem risco de
tocar duas vezes.
